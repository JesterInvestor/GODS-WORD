...
export default function StrongsTooltip({
  word,
  strongsRef,
  anchorElement,
  onClose,
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      console.log('[StrongsTooltip] Loading data for word:', word, 'ref:', strongsRef);
      setLoading(true);
      setError(null);
      if (strongsRef) {
        try {
          const data = await lookupStrongs(strongsRef);
          console.log('[StrongsTooltip] Lookup result:', data);
          ...
          console.error('[StrongsTooltip]', errorMsg);
          setError(errorMsg);
        }
      } else {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
        <h3 className="text-lg font-bold text-gray-800 dark:text-white">{word}</h3>
        <button
          onClick={onClose}
